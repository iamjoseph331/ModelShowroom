...
providers =  ['CPUExecutionProvider', 'GPUExecutionProvider', 'CUDAExecutionProvider']
ort_session = {}
...
def get_attributes(img):
    ...
    # preprocess
    img224 = cv2.resize(img,(256,256))
    img = letterbox(img,new_shape=256, auto=False)[0]
...
def predict(base64_image: str): 
    ...
    bbs = loosedetectxyxy(img=rgb_image, fx=1.3)
    res_bbs = []
    ms = []
    for bb in bbs:
...
if __name__ == '__main__':
    img = cv2.imread("/Users/macbook/Code/MVP/JosephPlatform/test_img/3.jpg")
    try:
